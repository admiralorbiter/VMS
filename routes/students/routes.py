...
def import_students_from_salesforce():
    ...
    try:
        ...
        if delta_info['requested_delta']:
            if is_delta:
                print(f"DELTA SYNC: Only fetching students modified after {delta_info['watermark_formatted']}")
            else:
        ...
        # Add delta filter
        delta_clause = delta_helper.build_date_filter(watermark) if is_delta and watermark else ""
        query = base_query.format(where_clause=where_clause, delta_clause=delta_clause, limit=chunk_size)
