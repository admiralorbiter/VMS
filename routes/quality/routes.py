...
def api_quality_score():
    ...
    if entity_type == "all":
        ...
        current_app.logger.info(f"Validation results count: {len(validation_results)}")
...
def api_clear_validation_data():
    ...
    base_run_ids = [run.id for run in ValidationRun.query.filter(*run_filters).all()]
    ...
    if history_to_delete > 0:
        ValidationHistory.query.filter(ValidationHistory.run_id.in_(run_ids)).delete(
            synchronize_session=False
        )
...
def api_run_multiple_validations():
    ...
    current_app.logger.info(
        f"Running multiple comprehensive validations: " f"entity_types={entity_types}"
    )
