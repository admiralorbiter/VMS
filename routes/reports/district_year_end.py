...
from models.event import Event, EventAttendance, EventStatus, EventStudentParticipation, EventType
...
def load_routes(bp):
    ...
    def refresh_district_year_end():
        ...
        try:
            ...
            return jsonify({"success": True, "message": f"Successfully refreshed data for {school_year[:2]}-{school_year[2:]} school year"})
    ...
    def district_year_end_detail(district_name):
        ...
        district_mapping = next((mapping for salesforce_id, mapping in DISTRICT_MAPPING.items() if mapping["name"] == district_name), None)

        # Try to get cached data first (use host_filter)
        cached_report = DistrictYearEndReport.query.filter_by(district_id=district.id, school_year=school_year, host_filter=host_filter).first()
        ...
        if cached_report:
            ...
            if cached_report.events_data:
                ...
                org_ids = db.session.query(VolunteerOrganization.organization_id).filter(VolunteerOrganization.volunteer_id.in_(volunteer_ids)).distinct().all()
        ...
        if host_filter == "prepkc":
            events_query = events_query.filter(
                db.or_(Event.session_host.ilike("%PREPKC%"), Event.session_host.ilike("%prepkc%"), Event.session_host.ilike("%PrepKC%"))
            )
        ...
        EventStudentParticipation.query.filter(EventStudentParticipation.event_id.in_(event_ids)).all()
        ...
        for event in events:
            ...
            if event.type == EventType.VIRTUAL_SESSION:
                ...
                district_student_ids = (
                    ...
                    .filter(EventStudentParticipation.event_id == event.id, EventStudentParticipation.status == "Attended", School.district_id == district.id)
                    .all()
                )
    ...
    def district_year_end_excel(district_name):
        ...
        # Add some formatting
        header_format = workbook.add_format({"bold": True, "bg_color": "#467599", "font_color": "white", "border": 1})
        ...
        if stats.get("event_types"):
            event_types_data = {"Event Type": list(stats["event_types"].keys()), "Count": list(stats["event_types"].values())}
    ...
    def get_filtered_stats(district_name):
        ...
        if unique_volunteers:
            org_ids = db.session.query(VolunteerOrganization.organization_id).filter(VolunteerOrganization.volunteer_id.in_(unique_volunteers)).distinct().all()
            unique_organization_count = len(org_ids)
        else:
    ...
    def get_filtered_participants(district_name):
        ...
        return jsonify({"volunteer_ids": list(filtered_volunteer_ids), "student_ids": list(filtered_student_ids), "event_ids": [event.id for event in events]})
    ...
    def manage_google_sheets():
        ...
        return render_template("reports/google_sheets_management.html", sheets=sheets, academic_year=academic_year, school_year=school_year)
    ...
    def add_google_sheet():
        ...
        try:
            ...
            return jsonify(
                {"success": True, "message": message, "sheet": {"academic_year": academic_year, "sheet_id": sheet_id, "created_by": current_user.username}}
            )
        except Exception as e:
            db.session.rollback()
            return jsonify({"success": False, "error": str(e)}), 500
    ...
    def get_google_sheets_api(academic_year):
        ...
        sheets = GoogleSheet.query.filter_by(academic_year=academic_year, purpose="district_reports").all()
        return jsonify({"sheets": [sheet.to_dict() for sheet in sheets], "academic_year": academic_year})
...
def generate_schools_by_level_data(district, events):
    ...
    schools_by_level = {"High": [], "Middle": [], "Elementary": [], None: []}  # For schools with no level or 'Other'
            ...
            if hasattr(event, "students") and hasattr(event, "volunteers"):
                ...
                if event_id:
                    volunteer_participations = EventParticipation.query.filter(
                        EventParticipation.event_id == event_id, EventParticipation.status.in_(["Attended", "Completed", "Successfully Completed"])
                    ).all()
            else:
...
def cache_district_stats_with_events(school_year, district_stats, host_filter="all"):
    ...
    for district_name, stats in district_stats.items():
        ...
        report = DistrictYearEndReport.query.filter_by(district_id=district.id, school_year=school_year, host_filter=host_filter).first()
...
def calculate_enhanced_district_stats(events, district_id):
    ...
    stats = {
        ...
        "virtual_sessions": {"classrooms_reached": 0, "unique_teachers": 0, "confirmed_teachers": 0, "registered_teachers": 0},
        "event_types": {},
    ...
    for event in events:
        ...
        if not is_virtual:
            district_student_ids = (
                ...
                .join(Student, EventStudentParticipation.student_id == Student.id)
                .join(School, Student.school_id == School.id)
                .filter(EventStudentParticipation.event_id == event.id, EventStudentParticipation.status == "Attended", School.district_id == district_id)
                .all()
            )
            event_student_ids = {student_id[0] for student_id in district_student_ids}
        ...
        if event_volunteer_ids:
            event_org_ids = (
                db.session.query(VolunteerOrganization.organization_id).filter(VolunteerOrganization.volunteer_id.in_(event_volunteer_ids)).distinct().all()
            )
            ...
            for org_id in event_org_ids:
                ...
                org_event_hours = sum(p.delivery_hours or 0 for p in volunteer_participations if p.volunteer_id in org_volunteer_ids)
