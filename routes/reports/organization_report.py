...
def load_routes(bp):
    ...
    def organization_report():
        ...
        for org, sessions, hours, volunteers in org_stats:
            org_data.append({"name": org.name, "id": org.id, "unique_sessions": sessions, "total_hours": round(hours or 0, 2), "unique_volunteers": volunteers})
    ...
    def organization_report_detail(org_id):
        ...
        volunteers_data = [
            {"id": v.id, "name": f"{v.first_name} {v.last_name}", "email": v.primary_email, "events": events, "hours": round(hours or 0, 2)}
        ...
        for event, volunteer, status, hours in detailed_inperson_events:
            ...
            if event_key not in inperson_events_by_event:
                inperson_events_by_event[event_key] = {"event": event, "volunteers": [], "total_hours": 0}
            inperson_events_by_event[event_key]["volunteers"].append(
                {"name": f"{volunteer.first_name} {volunteer.last_name}", "status": status, "hours": hours or 0}
            )
        ...
        detailed_virtual_events = (
            db.session.query(Event, Volunteer, EventParticipation.status, EventTeacher, TeacherAlias, SchoolAlias, DistrictAlias)
            .join(EventParticipation, Event.id == EventParticipation.event_id)
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .outerjoin(EventTeacher, Event.id == EventTeacher.event_id)
            ...
            .filter(
                ...
                EventParticipation.status.in_(["Attended", "Completed", "Successfully Completed", "Simulcast"]),
                Volunteer.exclude_from_reports is False,
                db.or_(TeacherAlias.id is None, TeacherAlias.exclude_from_reports is False),  # No teacher associated with event  # Teacher is not excluded
            )
            .order_by(Event.start_date, Event.title, Volunteer.last_name, Volunteer.first_name)
            .all()
        )
        ...
        for event, volunteer, status, event_teacher, teacher, school, district in detailed_virtual_events:
            ...
            if event_key not in virtual_events_by_event:
                virtual_events_by_event[event_key] = {"event": event, "volunteers": [], "classrooms": [], "unique_classroom_count": 0}
    ...
    def organization_report_excel():
        ...
        org_data = []
        for org, sessions, hours, volunteers in org_stats:
            org_data.append({"Organization": org.name, "Unique Sessions": sessions, "Total Hours": round(hours or 0, 2), "Unique Volunteers": volunteers})
    ...
    def organization_report_detail_excel(org_id):
        ...
        # Format volunteer data
        volunteers_data = [
            {"Name": f"{v.first_name} {v.last_name}", "Email": v.primary_email, "Events": events, "Hours": round(hours or 0, 2)}
            for v, events, hours in volunteer_stats
        ]
                ...
                if "classroom_details" in event and event["classroom_details"]:
                    for classroom in event["classroom_details"]:
                        if classroom["teacher_name"] not in unique_teachers_seen:
                            ...
                            teacher_session_counts_excel.get(classroom["teacher_name"], 1)
                            unique_classroom_details_data.append(
                                {"Teacher Name": classroom["teacher_name"], "School": classroom["school_name"], "District": classroom["district_name"]}
                            )
