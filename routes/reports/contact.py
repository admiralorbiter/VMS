...
def load_routes(bp):
    ...
    def contact_report():
        ...
        query = Event.query.filter(Event.start_date >= datetime.utcnow(), Event.status == EventStatus.CONFIRMED)  # Only future events
        ...
        return render_template("reports/contact_report.html", events=upcoming_events, event_stats=event_stats, search=search, sort=sort, order=order)
    ...
    def contact_report_detail(event_id):
        ...
        for status in participants_by_status:
            ...
            if sort == "name":
                participants.sort(key=lambda p: f"{p.volunteer.first_name} {p.volunteer.last_name}", reverse=(order == "desc"))
            elif sort == "title":
                participants.sort(key=lambda p: p.title or p.volunteer.title or "", reverse=(order == "desc"))
            elif sort == "email":
                participants.sort(key=lambda p: p.volunteer.primary_email or "", reverse=(order == "desc"))
            elif sort == "organization":
                participants.sort(key=lambda p: p.volunteer.organizations[0].name if p.volunteer.organizations else "", reverse=(order == "desc"))

        return render_template("reports/contact_report_detail.html", event=event, participants_by_status=participants_by_status, sort=sort, order=order)
