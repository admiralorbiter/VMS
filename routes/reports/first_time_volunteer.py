...
def load_routes(bp):
    ...
    def first_time_volunteer():
        ...
        cache = FirstTimeVolunteerReportCache.query.filter_by(school_year=school_year).first()
        if cache and not refresh:
            ...
            first_time_volunteers = (
                ...
                .filter(and_(Volunteer.first_volunteer_date >= start_date, Volunteer.first_volunteer_date <= end_date))
                .filter(
                    or_(
            ...
            for v, events_count, hours, org in all_volunteers_query:
                ...
                volunteer_events = (
                    db.session.query(Event, EventParticipation.delivery_hours, EventParticipation.status)
            ...
            # Save to cache
            if not cache:
                cache = FirstTimeVolunteerReportCache(school_year=school_year, report_data=report, last_updated=datetime.now())
    ...
    def export_first_time_volunteer():
        ...
        first_time_volunteers = (
            ...
            .outerjoin(
                Event,
                and_(
                    EventParticipation.event_id == Event.id, Event.start_date >= start_date, Event.start_date <= end_date, Event.status == EventStatus.COMPLETED
                ),
            )
            .outerjoin(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .outerjoin(Organization, VolunteerOrganization.organization_id == Organization.id)
        ...
        for v, events_count, hours, org in first_time_volunteers:
            ...
            volunteer_events = (
                ...
                .filter(
                    ...
                    Event.status == EventStatus.COMPLETED,
                    or_(
                        EventParticipation.status == "Attended", EventParticipation.status == "Completed", EventParticipation.status == "Successfully Completed"
                    ),
                )
                .order_by(Event.start_date)
                .all()
