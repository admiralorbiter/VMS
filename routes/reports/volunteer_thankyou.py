...
def load_routes(bp):
    ...
    def volunteer_thankyou():
        ...
        volunteer_stats = (
            db.session.query(
                Volunteer, db.func.sum(EventParticipation.delivery_hours).label("total_hours"), db.func.count(EventParticipation.id).label("total_events")
            )
        ...
        volunteer_stats = volunteer_stats.group_by(Volunteer.id).order_by(order_by).all()
        ...
        for v, hours, events in volunteer_stats:
            ...
            volunteer_data.append(
                {"id": v.id, "name": f"{v.first_name} {v.last_name}", "total_hours": volunteer_hours, "total_events": events, "organization": organization}
            )
