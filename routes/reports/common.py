...
DISTRICT_MAPPING = {
    ...
    "0015f00000JU4pVAAT": {  # Kansas City Kansas Public Schools
        ...
        "merge_with": None,
        "show": True,
        "aliases": ["Kansas City Kansas School District", "KANSAS CITY USD 500", "KCKPS (KS)"],
    },
    "0015f00000JU9ZEAA1": {"name": "Allen Village - District", "district_code": "48909", "merge_with": None, "show": True},  # Allen Village - District
    ...
    "0015f00000JVaPKAA1": {"name": "GUADALUPE CENTERS SCHOOLS", "district_code": "48902", "merge_with": None, "show": True},  # GUADALUPE CENTERS SCHOOLS
    "0015f00000KvuZTAAZ": {"name": "Center School District", "district_code": "48080", "merge_with": None, "show": True},  # Center School District
...
def generate_district_stats(school_year, host_filter="all"):
    ...
    for salesforce_id, mapping in DISTRICT_MAPPING.items():
        ...
        district_partner_conditions = [Event.district_partner.ilike(f"%{school.name}%") for school in schools]
        ...
        if "aliases" in mapping:
            for alias in mapping["aliases"]:
                ...
                district_partner_conditions.append(Event.districts.any(District.name.ilike(f"%{alias}%")))
        ...
        events_query = Event.query.filter(
            ...
            db.or_(Event.districts.contains(primary_district), Event.school.in_([school.id for school in schools]), *district_partner_conditions),
        ).distinct()
        ...
        for month in stats["monthly_breakdown"]:
            ...
            stats["monthly_breakdown"][month]["volunteer_hours"] = round(stats["monthly_breakdown"][month]["volunteer_hours"], 1)
...
def cache_district_stats(school_year, district_stats):
    ...
    for district_name, stats in district_stats.items():
        ...
        if district:
            ...
            report = DistrictYearEndReport.query.filter_by(district_id=district.id, school_year=school_year).first() or DistrictYearEndReport(
