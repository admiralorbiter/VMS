...
def load_routes(bp):
            ...
            if excluded_volunteer_participated:
                ...
                classroom_count = (
                    ...
                    .filter(
                        ...
                        EventTeacher.status.in_(
                            ["simulcast", "successfully completed"]
                        ),
    ...
    def organization_thankyou_detail_excel(org_id):
            ...
            virtual_events_data.append(
                {
                    ...
                    "Type": event.type.value if event.type else "Unknown",
                    "Volunteers": (
                        ", ".join(volunteer_names) if volunteer_names else "None"
                    ),
                    "Classrooms": event_data["classroom_count"],
                }
            )
        ...
        worksheet.conditional_format(
            "A1:B8", {"type": "no_blanks", "format": header_format}
        )
        ...
        if virtual_events_data:
            ...
            worksheet.conditional_format(
                "A1:F1", {"type": "no_blanks", "format": header_format}
            )

        # Event Type Breakdown Sheet
        if event_type_data:
            event_type_df = pd.DataFrame(event_type_data)
            event_type_df.to_excel(
                writer, sheet_name="Event Type Breakdown", index=False
            )
        ...
        if volunteers_data:
            ...
            worksheet.conditional_format(
                "A1:C1", {"type": "no_blanks", "format": header_format}
            )
        ...
        if detailed_participation_data:
            ...
            detailed_df.to_excel(
                writer, sheet_name="Detailed Participation", index=False
            )
        ...
        filename = (
            f"{org_name_clean}_{school_year[:2]}-{school_year[2:]}_Detail_Report.xlsx"
        )
