...
from models.event import Event, EventStatus, EventTeacher
...
def load_routes(bp):
    ...
    def organization_thankyou():
        ...
        org_stats = (
            ...
            .join(VolunteerOrganization, Organization.id == VolunteerOrganization.organization_id)
            ...
            .filter(
                Event.start_date >= start_date, Event.start_date <= end_date, EventParticipation.status.in_(["Attended", "Completed", "Successfully Completed"])
            )
        )
        ...
        for org, sessions, hours, volunteers in org_stats:
            ...
            org_data.append({"id": org.id, "name": org.name, "unique_sessions": sessions, "total_hours": org_hours, "unique_volunteers": volunteers})
    ...
    def organization_thankyou_excel():
        ...
        org_data = [
            {"Organization": org.name, "Unique Sessions": sessions, "Total Hours": round(hours or 0, 2), "Unique Volunteers": volunteers}
            for org, sessions, hours, volunteers in org_stats
        ]
    ...
    def organization_thankyou_detail(org_id):
        ...
        volunteer_stats = (
            db.session.query(
                Volunteer, db.func.count(db.distinct(Event.id)).label("event_count"), db.func.sum(EventParticipation.delivery_hours).label("total_hours")
            )
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .join(EventParticipation, Volunteer.id == EventParticipation.volunteer_id)
            .join(Event, EventParticipation.event_id == Event.id)
            .filter(
                ...
                Volunteer.exclude_from_reports is False,
            )
        ...
        volunteers_data = [
            {"id": v.id, "name": f"{v.first_name} {v.last_name}", "events": events, "hours": round(hours or 0, 2)} for v, events, hours in volunteer_stats
        ]
        ...
        for event_type, count, hours, volunteers in event_type_breakdown:
            ...
            event_type_data[type_name] = {"count": count, "hours": round(hours or 0, 2), "volunteers": volunteers}

        # Cancelled and No-show events - include both event-level cancellations and volunteer participation cancellations
        cancelled_events = (
            db.session.query(Event, db.func.count(db.distinct(EventParticipation.volunteer_id)).label("volunteer_count"))
            .join(EventParticipation, Event.id == EventParticipation.event_id)
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .filter(
        ...
        detailed_virtual_events = (
            db.session.query(Event, Volunteer, EventParticipation.status)
            .join(EventParticipation, Event.id == EventParticipation.event_id)
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .filter(
            ...
            .order_by(Event.start_date, Event.title, Volunteer.last_name, Volunteer.first_name)
            .all()
        )
        ...
        for event, volunteer, status in detailed_virtual_events:
            ...
            if event_key not in virtual_events_by_event:
                virtual_events_by_event[event_key] = {"event": event, "volunteers": [], "classroom_count": 0}
            virtual_events_by_event[event_key]["volunteers"].append({"name": f"{volunteer.first_name} {volunteer.last_name}", "status": status})
        ...
        for event_key, event_data in virtual_events_by_event.items():
            ...
            excluded_volunteer_participated = (
                db.session.query(EventParticipation)
                .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
                .filter(EventParticipation.event_id == event.id, Volunteer.exclude_from_reports is True)
                .first()
            )
            ...
            if excluded_volunteer_participated:
                ...
                classroom_count = (
                    db.session.query(db.func.count(db.distinct(EventTeacher.teacher_id)))
                    ...
                    .filter(
                        ...
                        Teacher.exclude_from_reports is False,
                    )
                    .scalar()
                )
        ...
        virtual_class_reach = (
            db.session.query(db.func.count(db.distinct(EventTeacher.teacher_id)).label("teacher_count"))
            .join(Event, EventTeacher.event_id == Event.id)
            .join(EventParticipation, Event.id == EventParticipation.event_id)
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
        ...
        detailed_participation = (
            db.session.query(Event, Volunteer, EventParticipation.status, EventParticipation.delivery_hours)
            .join(EventParticipation, Event.id == EventParticipation.event_id)
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .filter(
    ...
    def organization_thankyou_detail_excel(org_id):
        ...
        volunteers_data = [{"Name": f"{v.first_name} {v.last_name}", "Events": events, "Hours": round(hours or 0, 2)} for v, events, hours in volunteer_stats]
        ...
        for event_type, count, hours, volunteers in event_type_breakdown:
            type_name = event_type.value if event_type else "Unknown"
            event_type_data.append({"Event Type": type_name, "Event Count": count, "Total Hours": round(hours or 0, 2), "Volunteer Count": volunteers})
        ...
        detailed_participation = (
            ...
            .join(Volunteer, EventParticipation.volunteer_id == Volunteer.id)
            .join(VolunteerOrganization, Volunteer.id == VolunteerOrganization.volunteer_id)
            .filter(VolunteerOrganization.organization_id == org_id, Event.start_date >= start_date, Event.start_date <= end_date)
            .order_by(Event.start_date, Event.title, Volunteer.last_name, Volunteer.first_name)
            .all()
        )
