...
def load_routes(bp):
    ...
    def attendance_report():
        ...
        return render_template("reports/attendance_report.html", districts=districts, current_school_year=school_year)
    ...
    def get_attendance_data():
        ...
        query = Event.query.filter(Event.status == EventStatus.COMPLETED, Event.start_date >= start_date, Event.start_date <= end_date)
        ...
        for event in events:
            ...
            volunteer_participations = EventParticipation.query.filter(
                EventParticipation.event_id == event.id, EventParticipation.status.in_(["Attended", "Completed", "Successfully Completed"])
            ).all()
            ...
            student_participations = EventStudentParticipation.query.filter(
                EventStudentParticipation.event_id == event.id, EventStudentParticipation.status == "Attended"
            ).all()
