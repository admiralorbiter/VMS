...
def process_session_report_row(row, row_index, import_log, processed_events):
    ...
    try:
        ...
        if signup_role == "educator":
            ...
            # Sprint 2: Link teacher to event via EventTeacher FK
            if teacher:
                from services.teacher_service import ensure_event_teacher

                ensure_event_teacher(event.id, teacher.id)

            # Store educator name on event (accumulate if multiple)
            # This text field is now a cache — regenerated by sync_event_participant_fields
            if name:
