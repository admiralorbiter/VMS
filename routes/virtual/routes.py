    ...
    for teacher_name in teacher_names:
        ...
        if len(name_parts) >= 2:
            ...
            teacher = Teacher.query.filter(
                func.lower(Teacher.first_name) == func.lower(first_name), func.lower(Teacher.last_name) == func.lower(last_name)
            ).first()

            if not teacher:
                teacher = Teacher(first_name=first_name, last_name=last_name, middle_name="")
...
def find_or_create_volunteer(first_name, last_name, organization=None, people_of_color=None):
    ...
    volunteer = Volunteer.query.filter(
        func.lower(Volunteer.first_name) == func.lower(first_name), func.lower(Volunteer.last_name) == func.lower(last_name)
    ).first()
    ...
    if organization and not pd.isna(organization):
        ...
        vol_org = VolunteerOrganization.query.filter_by(volunteer_id=volunteer.id, organization_id=org.id).first()

        if not vol_org:
            # Create the missing link
            vol_org = VolunteerOrganization(volunteer_id=volunteer.id, organization_id=org.id, role="Professional", is_primary=True, status="Current")
...
def process_teacher_for_event(row, event, is_simulcast):
    ...
    for teacher_name in teacher_names:
        ...
        if len(name_parts) >= 2:
            ...
            event_teacher = EventTeacher.query.filter_by(event_id=event.id, teacher_id=teacher.id).first()
...
def purge_virtual():
    ...
    try:
        ...
        virtual_event_ids = db.session.query(Event.id).filter(Event.type == EventType.VIRTUAL_SESSION).all()
        ...
        if virtual_event_ids:
            event_teacher_deleted = EventTeacher.query.filter(EventTeacher.event_id.in_(virtual_event_ids)).delete(synchronize_session=False)

        # Delete event participations for virtual sessions (presenters/volunteers)
        event_participation_deleted = 0
        if virtual_event_ids:
            event_participation_deleted = EventParticipation.query.filter(EventParticipation.event_id.in_(virtual_event_ids)).delete(synchronize_session=False)

        # Then delete the virtual session events
        deleted_count = Event.query.filter_by(type=EventType.VIRTUAL_SESSION).delete(synchronize_session=False)
        ...
        orphaned_teachers = Teacher.query.filter(~Teacher.event_registrations.any()).all()
...
def list_events():
    ...
    try:
        events = Event.query.filter_by(type=EventType.VIRTUAL_SESSION).order_by(Event.start_date.desc()).all()
...
def get_event(event_id):
    ...
    try:
        event = Event.query.filter_by(id=event_id, type=EventType.VIRTUAL_SESSION).first_or_404()
        ...
        if "/" in date_str:
            ...
            if len(parts) >= 2:
                ...
                if time_obj:
                    ...
                    try:
                        return datetime(current_year, month, day, time_obj.hour, time_obj.minute)
                    except ValueError:
...
def get_or_create_district(name):
    ...
    # Handle 'Unknown District' case - find or create the specific 'Unknown' record
        ...
        if not district:
            ...
            print("INFO: Creating 'Unknown District'")
...
def import_sheet():
    try:
        ...
        df = pd.read_csv(pd.io.common.BytesIO(content), skiprows=3, dtype={"School Name": str})
                    ...
                    for lookup_row_index_pc, lookup_row_data_pc in enumerate(all_rows_for_lookup):
                        ...
                        if not pd.isna(lookup_date_str_pc):
                            try:
                                        ...
                                        if month_str_pc.isdigit() and day_str_pc.isdigit():
                                            ...
                                            if 1 <= month_pc <= 12 and 1 <= day_pc <= 31:
                                                ...
                                                if current_dt_utc_pc.month >= 7:  # Changed from >= 6 to >= 7
                                                    year_pc = (
                                                        current_dt_utc_pc.year if month_pc >= 7 else current_dt_utc_pc.year + 1
                                                    )  # Changed from >= 6 to >= 7
                                                else:
                                                    year_pc = (
                                                        current_dt_utc_pc.year - 1 if month_pc >= 7 else current_dt_utc_pc.year
                                                    )  # Changed from >= 6 to >= 7
                                                try:
                                ...
                                print(f"ERROR - Date parsing error: {e_pc}")
                        ...
                        if (
                            lookup_status_str_pc in ["successfully completed", "simulcast"]
        ...
        # Summary of results
        print("\n=== Import Complete ===")
        print("Success: {success_count} events processed (new + updated)")
        ...
        try:
            ...
            print("PASS: Virtual session caches invalidated - reports will show fresh data")
        except Exception as e:
...
def get_or_create_school(name, district=None):
    ...
    try:
        ...
        school = School.query.filter(func.lower(School.name) == func.lower(name)).first()
