...
from models.event import (  # Add Enums
    CancellationReason,
    Event,
    EventFormat,
    EventStatus,
...
from models.volunteer import (  # Need Skill for parsing and volunteer participation
    EventParticipation,
    Skill,
    Volunteer,
)
...
def _create_event_from_salesforce(sf_event_data, event_districts):
    ...
    new_event = Event(
        ...
        title=sf_event_data.get("Name", "").strip()
        or f"Untitled Event {event_sf_id}",  # Ensure title is not empty
    )
        ...
        for sf_event_data in unaffiliated_events_data:
            ...
            try:
                ...
                local_event = (
                    ...
                    .filter(Event.salesforce_id == event_sf_id)
                    .first()
                )
                ...
                if local_event:
                    ...
                    if (
                        sf_event_data.get("Session_Host__c")
                        and sf_event_data.get("Session_Host__c")
                        != local_event.session_host
                    ):
                        local_event.session_host = sf_event_data.get("Session_Host__c")
                        updated_fields.append("session_host")

                    if (
                        sf_event_data.get("Description__c")
                        and sf_event_data.get("Description__c")
                    ...
                    if (
                        sf_event_data.get("Location_Information__c")
                        and sf_event_data.get("Location_Information__c")
                        != local_event.location
                    ):
                        local_event.location = sf_event_data.get(
                    ...
                    if not local_event.districts:
                        ...
                        if student_sf_ids:
                            ...
                            if event_districts:
                                ...
                                print(
                                    f"UPDATED existing Event {event_sf_id} ({local_event.title}) with districts: {[d.name for d in event_districts]}"
                                )
                    ...
                    if updated_fields:
                        ...
                        updated_count += 1
                        print(
                            f"UPDATED existing Event {event_sf_id} ({local_event.title}) with fields: {updated_fields}"
                        )
                    else:
                        print(
                            f"Existing event {event_sf_id} ({local_event.title}) - no updates needed"
                        )
                ...
                if not local_event:
                    ...
                    try:
                        ...
                        if event_districts:
                            ...
                            print(
                                f"CREATED new Event {event_sf_id} ({new_event.title}) affiliated with districts: {[d.name for d in event_districts]}"
                            )
                        else:
                            district_map_details[event_sf_id] = [
                                "No district association - volunteer-only event"
                            ]
                            print(
                                f"CREATED new Event {event_sf_id} ({new_event.title}) as volunteer-only event (no district association)"
                            )
                        ...
                        db.session.rollback()
                        error_msg = (
                            f"Error CREATING event {event_sf_id}: {str(creation_error)}"
                        )
                        print(error_msg)
        ...
        if processed_event_sf_ids:
            ...
            for i in range(0, len(processed_event_sf_ids), batch_size):
                ...
                try:
                    ...
                    print(
                        f"Found {len(participant_rows)} volunteer participation records for batch {i//batch_size + 1}"
                    )
                    ...
                    print(error_msg)
                    errors.append(error_msg)
                    participant_error += len(
                        batch_sf_ids
                    )  # Count all events in failed batch as errors

            print(
                f"Successfully processed {participant_success} volunteer participations with {participant_error} errors"
            )
        ...
        if processed_event_sf_ids:
            ...
            for i in range(0, len(processed_event_sf_ids), batch_size):
                ...
                try:
                    ...
                    student_participant_rows = student_participant_result.get(
                        "records", []
                    )
                    print(
                        f"Found {len(student_participant_rows)} student participation records for batch {i//batch_size + 1}"
                    )
                    for row in student_participant_rows:
                        student_participant_success, student_participant_error = (
                            process_student_participation_row(
                                row,
                                student_participant_success,
                                student_participant_error,
                                errors,
                            )
                        )
                except Exception as batch_error:
            ...
            print(
                f"Successfully processed {student_participant_success} student participations with {student_participant_error} errors"
            )
