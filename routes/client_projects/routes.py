...
def create():
    ...
    try:
        ...
        projects = ClientProject.query.order_by(ClientProject.created_at.desc()).all()
        return render_template(
            "client_connected_projects/partials/project_table.html",
            projects=projects,
            ProjectStatus=ProjectStatus,
        )
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                # Process contact information
                contacts_str = str(
                    row.get("Primary Org Contacts (with volunteer hours)", "")
                )
                contacts = []
                ...
                for contact in contacts_str.split("\n"):
                    ...
                    if contact:
                        ...
                        if match:
                            ...
                            contacts.append(
                                {"name": name.strip(), "hours": hours.strip()}
                            )
                        else:
                            # Handle cases without hours specified
                            contacts.append(
                                {"name": contact.split("(")[0].strip(), "hours": "0"}
                            )
                ...
                project = ClientProject(
                    ...
                    number_of_students=parse_student_number(
                        row.get("Number of Students", 0)
                    ),
                )
