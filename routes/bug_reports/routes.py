...
def submit_report():
    ...
    try:
        ...
        report = BugReport(type=int(report_type), description=description, page_url=page_url, page_title=page_title, submitted_by_id=current_user.id)

        db.session.add(report)
        db.session.commit()

        return jsonify({"success": True, "message": "Thank you for your report. We will look into this."})

    except Exception as e:
        db.session.rollback()
        # Log the error for debugging (you should have proper logging set up)
        print(f"Error submitting bug report: {str(e)}")
        return jsonify({"success": False, "message": "An error occurred while submitting your report. Please try again."}), 500
