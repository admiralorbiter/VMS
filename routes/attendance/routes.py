...
def purge_attendance():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"Successfully purged {purge_type} data"})
...
def attendance_details():
    ...
    event_types = db.session.query(Event.type).filter(Event.start_date >= start, Event.start_date <= end).distinct().all()
    event_types = [et[0] for et in event_types if et[0]]
    event_types = [{"value": et.value, "label": et.value.replace("_", " ").title()} for et in event_types]
