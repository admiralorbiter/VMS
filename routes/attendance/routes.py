...
def attendance_details():
    ...
    events = (
        Event.query.filter(Event.start_date >= start, Event.start_date <= end)
...
def attendance_details_events_json():
    ...
    if types:
        ...
        enum_types = [
            EventType(t)
            for t in types
            if t in EventType.__members__.values() or t in [e.value for e in EventType]
        ]
