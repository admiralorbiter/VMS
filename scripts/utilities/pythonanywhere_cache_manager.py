...
def refresh_specific_cache(cache_type):
    ...
    try:
        from utils.cache_refresh_scheduler import (
            refresh_specific_cache as refresh_cache,
        )
        ...
        duration = (datetime.now(timezone.utc) - start_time).total_seconds()
        logger.info(
            f"{cache_type.title()} caches refreshed successfully in {duration:.2f} seconds"
        )
        return True
...
def should_refresh_cache():
    ...
    try:
        ...
        last_refresh = datetime.fromisoformat(
            status["last_refresh"].replace("Z", "+00:00")
        )
...
def main():
    ...
    refresh_parser.add_argument(
        "--force", action="store_true", help="Force refresh even if recently refreshed"
    )
    refresh_parser.add_argument(
        "--type",
        choices=[
