...
def optimized_query_active_volunteers(start_date, end_date, title_contains=None):
        ...
        volunteers.append(
            {
                ...
                "last_event_type": (
                    row.last_event_type.value if row.last_event_type else None
                ),
    ...
    print(
        f"✅ Optimized query completed in {duration:.1f}ms, returned {len(volunteers)} volunteers"
    )
...
def test_performance():
    ...
    with app.app_context():
        ...
        first_time_volunteers = optimized_query_first_time_volunteers(
            start_date, end_date
        )
        ...
        if first_time_volunteers:
            print(
                f"Sample first-time volunteer: {first_time_volunteers[0]['name']} - {first_time_volunteers[0]['total_events']} events"
            )
