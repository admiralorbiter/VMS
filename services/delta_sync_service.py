...
class DeltaSyncHelper:
    ...
    def __init__(self, sync_type: str, buffer_hours: int = None):
        ...
        self.buffer_hours = buffer_hours if buffer_hours is not None else self.DEFAULT_BUFFER_HOURS
    ...
    def build_date_filter(self, watermark: datetime, field: str = 'LastModifiedDate') -> str:
        """
    ...
    def get_delta_info(self, request_args: dict) -> dict:
        ...
        return {
            ...
            'watermark': watermark,
            'watermark_formatted': self.format_watermark_for_soql(watermark) if watermark else None,
...
def add_delta_filter_to_query(
    ...
    if delta_info['actual_delta'] and delta_info['watermark']:
        ...
        if 'ORDER BY' in query_upper:
            ...
            modified_query = base_query + helper.build_date_filter(delta_info['watermark'], date_field)
...
def create_sync_log_with_watermark(
    ...
    sync_log = SyncLog(
        ...
        last_sync_watermark=datetime.now(timezone.utc) if status in ('success', 'partial') else None,
    )
