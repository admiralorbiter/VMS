...
class OrganizationDetailCache(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("organization_id", "school_year", name="uq_org_detail_cache"),)
...
class FirstTimeVolunteerReportCache(db.Model):
    ...
    report_data = db.Column(db.JSON, nullable=False)
    last_updated = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    __table_args__ = (db.UniqueConstraint("school_year", name="uq_first_time_volunteer_report_cache"),)
...
class VirtualSessionReportCache(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("virtual_year", "date_from", "date_to", name="uq_virtual_session_report_cache"),
...
class VirtualSessionDistrictCache(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("district_name", "virtual_year", "date_from", "date_to", name="uq_virtual_session_district_cache"),
