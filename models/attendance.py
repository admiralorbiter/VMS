...
class EventAttendanceDetail(db.Model):
    ...
    num_classrooms = db.Column(
        db.Integer, nullable=True
    )  # Number of classrooms or tables used
    rotations = db.Column(
        db.Integer, nullable=True
    )  # Number of rotations for the event
    students_per_volunteer = db.Column(
        db.Integer, nullable=True
    )  # Average students per volunteer ratio
    total_students = db.Column(
        db.Integer, nullable=True
    )  # Total number of students who attended

    # Salesforce integration flag
    attendance_in_sf = db.Column(
        db.Boolean, default=False
    )  # Whether attendance was recorded in Salesforce

    # Event categorization fields
    pathway = db.Column(
        db.String(128), nullable=True
    )  # Career pathway focus (e.g., STEM, Arts, etc.)
    groups_rotations = db.Column(
        db.String(128), nullable=True
    )  # Group rotation information
    ...
    is_stem = db.Column(
        db.Boolean, default=False
    )  # Whether this is a STEM-focused event

    # External link to attendance tracking system
    attendance_link = db.Column(
