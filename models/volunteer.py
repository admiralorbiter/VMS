...
from models.contact import Contact, ContactTypeEnum, EducationEnum, Enum, FormEnum, LocalStatusEnum, SkillSourceEnum
...
class Volunteer(Contact):
    ...
    def total_times_volunteered(self):
        ...
        participation_count = EventParticipation.query.filter(
            ...
            or_(EventParticipation.status == "Attended", EventParticipation.status == "Completed", EventParticipation.status == "Successfully Completed"),
        ).count()
    ...
    def active_histories(self):
        ...
        """
        return History.query.filter_by(volunteer_id=self.id, is_deleted=False).order_by(History.activity_date.desc()).all()
    ...
    def calculate_local_status(self):
        ...
        try:
            ...
            personal_addr = next((addr for addr in self.addresses if addr.type == ContactTypeEnum.personal), None)
    ...
    def volunteer_organizations(cls):
        ...
        """
        return relationship("VolunteerOrganization", back_populates="volunteer", cascade="all, delete-orphan", passive_deletes=True, overlaps="organizations")
    ...
    def connector(cls):
        ...
        """
        return relationship("ConnectorData", uselist=False, back_populates="volunteer", cascade="all, delete-orphan", single_parent=True)
