...
class EventAttendance(db.Model):
    ...
    event = db.relationship("Event", backref=db.backref("attendance", uselist=False, cascade="all, delete-orphan"), single_parent=True)
...
class Event(db.Model):
    ...
    comments = db.relationship("EventComment", backref="event", lazy="dynamic", cascade="all, delete-orphan")  # Comments are deleted when event is deleted

    districts = db.relationship("District", secondary=event_districts, backref=db.backref("events", lazy="dynamic"))

    skills = db.relationship("Skill", secondary=event_skills, backref=db.backref("events", lazy="dynamic"))

    # Enhanced teacher tracking
    teacher_registrations = db.relationship("EventTeacher", back_populates="event", cascade="all, delete-orphan")

    teachers = db.relationship("Teacher", secondary="event_teacher", back_populates="events", viewonly=True)  # Use teacher_registrations for modifications

    # Add this to your Event class relationships section
    school_obj = db.relationship("School", foreign_keys=[school], backref=db.backref("events", lazy="dynamic"))

    # Add one-to-one relationship to EventAttendanceDetail
    attendance_detail = db.relationship("EventAttendanceDetail", back_populates="event", uselist=False, cascade="all, delete-orphan")
    ...
    def merge_duplicate(self, data):
        ...
        if name and role:
            if role == "educator":
                ...
                volunteer = Volunteer.query.filter(Volunteer.first_name == first_name, Volunteer.last_name == last_name).first()

                if not volunteer:
                    # Create new volunteer with empty middle name
                    volunteer = Volunteer(first_name=first_name, last_name=last_name, middle_name="")  # Explicitly set empty string
                    ...
                    if company:
                        ...
                        vol_org = VolunteerOrganization(volunteer=volunteer, organization=org, role="Professional", is_primary=True)
    ...
    def validate_status_transition(self):
        ...
        if self._previous_status in invalid_transitions and self.status in invalid_transitions[self._previous_status]:
            raise ValueError(f"Invalid status transition from {self._previous_status} to {self.status}")
    ...
    def add_volunteer(self, volunteer, participant_type="Volunteer", status="Confirmed"):
        ...
        participation = EventParticipation.query.filter_by(volunteer_id=volunteer.id, event_id=self.id).first()

        if not participation:
            participation = EventParticipation(volunteer_id=volunteer.id, event_id=self.id, participant_type=participant_type, status=status)
    ...
    def confirmed_teacher_count(self):
        ...
        return EventTeacher.query.filter(EventTeacher.event_id == self.id, EventTeacher.attendance_confirmed_at.isnot(None)).count()
