...
class Teacher(Contact):
    ...
    school_id = db.Column(String(255), nullable=True, index=True, comment="Maps to npsp__Primary_Affiliation__c in external systems")
    ...
    event_registrations = db.relationship("EventTeacher", back_populates="teacher", cascade="all, delete-orphan")

    events = db.relationship("Event", secondary="event_teacher", back_populates="teachers", viewonly=True)  # Use event_registrations for modifications

    school = relationship("School", foreign_keys=[school_id], primaryjoin="Teacher.school_id == School.id", backref=db.backref("teachers", lazy="dynamic"))

    __table_args__ = (
        db.CheckConstraint("connector_end_date >= connector_start_date", name="check_date_range"),
    ...
    def update_from_csv(self, data):
        ...
        if phone_number:
            existing_phone = Phone.query.filter_by(contact_id=self.id, number=phone_number, primary=True).first()
        ...
        if email_address:
            ...
            existing_email = Email.query.filter_by(contact_id=self.id, email=email_address, primary=True).first()
    ...
    def upcoming_events(self):
        ...
        return [reg.event for reg in self.event_registrations.all() if reg.event.start_date > now]

    @property
    def past_events(self):
        """Get teacher's past events."""
        return [reg.event for reg in self.event_registrations.all() if reg.event and reg.event.start_date < datetime.now()]
