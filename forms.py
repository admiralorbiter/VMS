...
from models.contact import EducationEnum, GenderEnum, LocalStatusEnum, RaceEthnicityEnum, SalutationEnum, SuffixEnum
...
class VolunteerForm(FlaskForm):
    ...
    gender = SelectField("Gender", choices=GenderEnum.choices(), validators=[Optional()])
    race_ethnicity = SelectField("Race/Ethnicity", choices=RaceEthnicityEnum.choices(), validators=[Optional()])
    education = SelectField("Education", choices=EducationEnum.choices(), validators=[Optional()])


class EventForm(FlaskForm):
    title = StringField("Event Title", validators=[DataRequired()])
    type = SelectField(
        "Event Type", choices=[("", "Select Type")] + [(t.value, t.value.replace("_", " ").title()) for t in EventType], validators=[DataRequired()]
    )
    format = SelectField(
        "Format", choices=[("", "Select Format")] + [(f.value, f.value.replace("_", " ").title()) for f in EventFormat], validators=[DataRequired()]
    )
    ...
    status = SelectField("Status", choices=[("", "Select Status")] + [(s.value, s.value) for s in EventStatus], validators=[DataRequired()])
    description = TextAreaField("Description", validators=[Optional()])
    volunteers_needed = IntegerField("Volunteers Needed", validators=[Optional(), NumberRange(min=0)], default=0)
