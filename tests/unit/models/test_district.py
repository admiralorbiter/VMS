...
def test_new_district(app):
    ...
    with app.app_context():
        district = District(name="Test School District", district_code="4045", salesforce_id="0015f00000JVZsFAAX")
...
def test_district_without_code(app):
    ...
    with app.app_context():
        district = District(name="District Without Code", salesforce_id="0015f00000JVZsGAAX")
        db.session.add(district)
        db.session.commit()
...
@pytest.mark.parametrize(
    ...
    [({"salesforce_id": "0015f00000JVZsHAAX", "name": None}, "NOT NULL constraint failed: district.name")],  # Explicitly set name to None
)
...
def test_unique_salesforce_id(app):
    ...
    with app.app_context():
        district1 = District(name="Original District", salesforce_id="0015f00000JVZsIAAX")
        db.session.add(district1)
        db.session.commit()

        duplicate_district = District(name="Duplicate District", salesforce_id="0015f00000JVZsIAAX")  # Same salesforce_id
...
def test_district_code_format(app):
    ...
    with app.app_context():
        for code in test_codes:
            district = District(name="District Test", district_code=code)
            db.session.add(district)
            db.session.commit()
...
def test_district_schools_query(app):
    ...
    with app.app_context():
        district = District(name="Query Test District", salesforce_id="0015f00000QUERY01")
        db.session.add(district)
        db.session.commit()
...
def test_district_null_salesforce_id(app):
    ...
    with app.app_context():
        ...
        # Add a school
        school = School(id="0015f00000NULLTST", name="School with No SF District", district_id=district.id)
        db.session.add(school)
        db.session.commit()
