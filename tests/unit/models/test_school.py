...
def test_new_school(app):
    ...
    with app.app_context():
        school = School(id="0015f00000TEST123", name="Test High School", normalized_name="TEST HIGH SCHOOL", school_code="4045")
...
def test_school_district_relationship(app):
    ...
    with app.app_context():
        ...
        school = School(id="0015f00000TEST456", name="District Test School", district_id=district.id, salesforce_district_id="0015f00000JVZsFAAX")
...
def test_school_cascade_delete(app):
    ...
    with app.app_context():
        ...
        district = District(name="Test District", district_code="9999", salesforce_id="0015f00000TEST999")
        db.session.add(district)
        db.session.commit()

        # Create school
        school = School(id="0015f00000TEST789", name="Cascade Test School", district_id=district.id, salesforce_district_id="0015f00000TEST999")
        db.session.add(school)
        db.session.commit()
...
def test_school_salesforce_district_id_mismatch(app):
    ...
    with app.app_context():
        ...
        school = School(id="0015f00000TEST456", name="Mismatch Test School", district_id=district.id, salesforce_district_id="DIFFERENT_ID")  # Mismatched ID
        db.session.add(school)
        db.session.commit()
...
def test_multiple_schools_per_district(app):
    ...
    with app.app_context():
        district = District(name="Multi-School District", salesforce_id="0015f00000MULTI01")
        db.session.add(district)
        db.session.commit()
        ...
        schools = []
        for i in range(3):
            school = School(id=f"0015f00000TEST{i}", name=f"School {i}", district_id=district.id, salesforce_district_id=district.salesforce_id)
