...
from models.contact import Address, ContactTypeEnum, Email, GenderEnum, RaceEthnicityEnum
...
def test_student_school_relationship(app, test_school):
    ...
    with app.app_context():
        ...
        student = Student(
            first_name="School", last_name="Student", racial_ethnic=RaceEthnicityEnum.white, school_id=school.id, active=True  # Add active status
        )
...
def test_student_inheritance(app):
    ...
    with app.app_context():
        student = Student(first_name="Inheritance", last_name="Test", racial_ethnic=RaceEthnicityEnum.white)
...
def test_student_required_fields(app):
    ...
    with app.app_context():
        ...
        with pytest.raises(Exception):
            student = Student(first_name=None, last_name="Student")  # None triggers SQLAlchemy's nullable check
            db.session.add(student)
            db.session.commit()
...
def test_student_demographic_fields(app):
    ...
    with app.app_context():
        student = Student(
            first_name="Demo", last_name="Student", gender=GenderEnum.female, racial_ethnic=RaceEthnicityEnum.multi_racial, birthdate=date(2010, 1, 1)
        )
        db.session.add(student)
        db.session.commit()
...
def test_student_contact_methods(app):
    ...
    with app.app_context():
        ...
        # Add email and test
        student.emails.append(db.session.merge(Email(email="student@test.com", primary=True, type=ContactTypeEnum.personal)))
...
def test_student_address_handling(app):
    ...
    with app.app_context():
        ...
        # Add address
        address = Address(address_line1="123 Test St", city="Test City", state="TS", zip_code="12345", type=ContactTypeEnum.personal, primary=True)
