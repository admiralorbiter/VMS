...
def test_salesforce_urls(test_contact):
    ...
    assert (
        test_contact.salesforce_contact_url
        == "https://prep-kc.lightning.force.com/lightning/r/Contact/003TESTID123456789/view"
    )
    ...
    assert (
        test_contact.salesforce_account_url
        == "https://prep-kc.lightning.force.com/lightning/r/Account/001TESTID123456789/view"
    )


def test_contact_type_polymorphism():
    """Test contact type polymorphism"""
    contact = Contact(type="contact", first_name="Base", last_name="Contact")
...
def test_contact_invalid_data(app, invalid_data):
    ...
    with app.app_context():
        ...
        with pytest.raises(
            Exception
        ):  # SQLAlchemy will raise an error for nullable=False violations
...
def test_address_with_line2(app):
    ...
    with app.app_context():
        contact = Contact(type="contact", first_name="Jane", last_name="Smith")
...
def test_primary_validation(app):
    ...
    with app.app_context():
        contact = Contact(type="contact", first_name="Test", last_name="Validation")
        ...
        with pytest.raises(
            ValueError, match="Contact cannot have multiple primary emails"
        ):
        ...
        # Should raise ValueError
        with pytest.raises(
            ValueError, match="Contact cannot have multiple primary phones"
        ):
