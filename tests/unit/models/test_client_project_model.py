...
def test_project_status_enum(app):
    with app.app_context():
        ...
        for status in ProjectStatus:
            project = ClientProject(status=status.value, teacher="Test Teacher", district="Test District", organization="Test Org")
            db.session.add(project)
            db.session.commit()
...
def test_json_field_primary_contacts(app):
    with app.app_context():
        contacts = [{"name": "Contact 1", "hours": 10}, {"name": "Contact 2", "hours": 15}, {"name": "Contact 3", "hours": 5}]
        project = ClientProject(
            status=ProjectStatus.PLANNING.value, teacher="Test Teacher", district="Test District", organization="Test Org", primary_contacts=contacts
        )
        db.session.add(project)
        db.session.commit()
...
def test_timestamp_behavior(app):
    with app.app_context():
        project = ClientProject(status=ProjectStatus.IN_PROGRESS.value, teacher="Test Teacher", district="Test District", organization="Test Org")
        db.session.add(project)
        db.session.commit()
...
def test_project_status_workflow(app):
    with app.app_context():
        # Start with planning
        project = ClientProject(status=ProjectStatus.PLANNING.value, teacher="Test Teacher", district="Test District", organization="Test Org")
        db.session.add(project)
        db.session.commit()
