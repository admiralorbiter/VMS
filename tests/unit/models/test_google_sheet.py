...
def test_user(app):
    with app.app_context():
        user = User(username="sheetuser", email="sheetuser@example.com", password_hash="hash", first_name="Sheet", last_name="User", is_admin=False)
        db.session.add(user)
        db.session.commit()
        yield user
...
def test_create_google_sheet(app, test_user, encryption_key):
    with app.app_context():
        ...
        sheet = GoogleSheet(academic_year="2023-2024", sheet_id=sheet_id, created_by=test_user.id)
...
def test_multiple_district_reports_allowed(app, encryption_key):
    ...
    with app.app_context():
        ...
        sheet1 = GoogleSheet(academic_year="2023-2024", sheet_id="sheet1", purpose="district_reports")
        db.session.add(sheet1)
        db.session.commit()

        # Should be able to create another district_reports sheet with same academic year
        sheet2 = GoogleSheet(academic_year="2023-2024", sheet_id="sheet2", purpose="district_reports")
        db.session.add(sheet2)
        db.session.commit()  # Should not raise an exception

        # Should be able to create a third one too
        sheet3 = GoogleSheet(academic_year="2023-2024", sheet_id="sheet3", purpose="district_reports")
...
def test_virtual_sessions_basic_functionality(app, encryption_key):
    ...
    with app.app_context():
        ...
        virtual_sheet = GoogleSheet(academic_year="2023-2024", sheet_id="virtual1", purpose="virtual_sessions")
...
def test_user_relationship(app, test_user, encryption_key):
    with app.app_context():
        sheet = GoogleSheet(academic_year="2023-2024", sheet_id="test_sheet", created_by=test_user.id)
        db.session.add(sheet)
        db.session.commit()
...
def test_to_dict_method(app, test_user, encryption_key):
    with app.app_context():
        sheet = GoogleSheet(academic_year="2023-2024", sheet_id="test_sheet_for_dict", created_by=test_user.id)
        db.session.add(sheet)
        db.session.commit()
...
def test_repr_method(app, encryption_key):
    with app.app_context():
        ...
        virtual_sheet = GoogleSheet(academic_year="2024-2025", sheet_id="virtual_test_sheet", purpose="virtual_sessions")
        db.session.add(virtual_sheet)
        db.session.commit()
