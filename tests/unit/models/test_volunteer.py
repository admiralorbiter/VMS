...
def test_volunteer_skills(app, test_volunteer, test_skill):
    ...
    with app.app_context():
        ...
        volunteer_skill = VolunteerSkill(volunteer_id=test_volunteer.id, skill_id=test_skill.id, source=SkillSourceEnum.user_selected, interest_level="High")
        ...
        vs = db.session.query(VolunteerSkill).filter_by(volunteer_id=test_volunteer.id, skill_id=test_skill.id).first()
...
def test_active_histories(app, test_volunteer):
    ...
    with app.app_context():
        ...
        active_history = History(volunteer_id=test_volunteer.id, activity_date=datetime.now(), activity_type="Test Activity", is_deleted=False)
        deleted_history = History(volunteer_id=test_volunteer.id, activity_date=datetime.now(), activity_type="Deleted Activity", is_deleted=True)
...
def test_volunteer_cascade_delete(app, test_volunteer):
    ...
    with app.app_context():
        ...
        connector = ConnectorData(volunteer_id=test_volunteer.id, active_subscription=ConnectorSubscriptionEnum.ACTIVE, role="Test Role")
        ...
        vol_skill = VolunteerSkill(volunteer_id=test_volunteer.id, skill_id=skill.id, source=SkillSourceEnum.user_selected)
        ...
        assert db.session.query(VolunteerSkill).filter_by(volunteer_id=vol_skill_id).first() is None
...
def test_volunteer_validation(app):
    ...
    with app.app_context():
        with pytest.raises(ValueError):
            volunteer = Volunteer(
                first_name="Test", last_name="Volunteer", education="InvalidEducation"  # This should raise ValueError since it's not an EducationEnum
            )
...
def test_volunteer_contact_info(app, test_volunteer):
    ...
    with app.app_context():
        # Add email
        email = Email(email="test@example.com", type=ContactTypeEnum.personal, primary=True, contact_id=test_volunteer.id)

        # Add phone
        phone = Phone(number="123-456-7890", type=ContactTypeEnum.personal, primary=True, contact_id=test_volunteer.id)
...
def test_volunteer_engagement_tracking(app, test_volunteer, test_event):
    ...
    with app.app_context():
        ...
        # Create engagement
        engagement = Engagement(volunteer_id=test_volunteer.id, engagement_date=date.today(), engagement_type="Meeting", notes="Initial meeting")
        db.session.add(engagement)
        db.session.commit()

        # Create event participation with valid event_id
        participation = EventParticipation(volunteer_id=test_volunteer.id, event_id=test_event.id, status="Confirmed", delivery_hours=2.5)
...
def test_local_status_calculation(app, test_volunteer):
    ...
    with app.app_context():
        ...
        kc_address = Address(contact_id=test_volunteer.id, address_line1="123 Main St", city="Kansas City", state="MO", zip_code="64111", primary=True)
...
def test_education_validation(app):
    ...
    with app.app_context():
        with pytest.raises(ValueError):
            volunteer = Volunteer(first_name="Test", last_name="Volunteer", education="Invalid Value")  # Should raise ValueError
            db.session.add(volunteer)
            db.session.commit()
...
def test_connector_data_constraints(app, test_volunteer):
    ...
    with app.app_context():
        ...
        connector1 = ConnectorData(volunteer_id=test_volunteer.id, active_subscription=ConnectorSubscriptionEnum.ACTIVE, user_auth_id="ABC1234")
        ...
        with pytest.raises(IntegrityError):  # Changed from generic Exception
            connector2 = ConnectorData(
                volunteer_id=test_volunteer.id, active_subscription=ConnectorSubscriptionEnum.ACTIVE, user_auth_id="ABC1234"  # Duplicate user_auth_id
            )
        ...
        with pytest.raises(IntegrityError):  # Changed from generic Exception
            connector3 = ConnectorData(volunteer_id=test_volunteer.id, active_subscription=ConnectorSubscriptionEnum.ACTIVE, user_auth_id="XYZ9876")
