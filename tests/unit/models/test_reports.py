...
def test_organization(app):
    with app.app_context():
        org = Organization(
            name="Test Organization", description="Test Description", type="Business"
        )
        db.session.add(org)
        db.session.commit()
        yield org
...
def test_organization_report(app, test_organization):
    with app.app_context():
        ...
        report = OrganizationReport(
            ...
            school_year="2024",
            summary_stats=summary_stats,
            in_person_events_data=in_person_events,
        )
        db.session.add(report)
        db.session.commit()

        assert report.id is not None
        assert str(report.organization_id) == str(
...
def test_organization_summary_cache(app):
    with app.app_context():
        organizations_data = [
            {"id": 1, "name": "Org 1", "event_count": 10},
            {"id": 2, "name": "Org 2", "event_count": 15},
        ]

        cache = OrganizationSummaryCache(
            school_year="2024", organizations_data=organizations_data
        )
...
def test_organization_detail_cache(app):
    with app.app_context():
        in_person_events = [
            {"id": 1, "title": "In-Person Event 1"},
            {"id": 2, "title": "In-Person Event 2"},
        ]
...
def test_unique_constraints(app, test_district):
    with app.app_context():
        ...
        report2 = DistrictYearEndReport(
            district_id=test_district.id,
            school_year="2024",
            host_filter="all",
            report_data={"test": "data2"},
        )
...
def test_json_field_validation(app, test_district):
    with app.app_context():
        ...
        complex_data = {
            "nested": {
                "level1": {"level2": ["item1", "item2"], "numbers": [1, 2, 3, 4, 5]}
            },
            "arrays": [
...
def test_foreign_key_relationships(app, test_district, test_organization):
    with app.app_context():
        ...
        district_report = DistrictYearEndReport(
            district_id=test_district.id,
            school_year="2024",
            host_filter="all",
            report_data={"test": "data"},
