...
def test_event(app):
    with app.app_context():
        event = Event(title="Attendance Test Event", type="IN_PERSON", start_date=datetime.now(), status="Confirmed", volunteers_needed=1)
...
def test_create_attendance_detail(app, test_event):
    ...
    with app.app_context():
        attendance = EventAttendanceDetail(event_id=test_event.id, total_students=25, num_classrooms=3, rotations=2, is_stem=True, pathway="STEM")
...
def test_calculate_students_per_volunteer(app, test_event):
    ...
    with app.app_context():
        ...
        attendance = EventAttendanceDetail(event_id=test_event.id, total_students=30, num_classrooms=3, rotations=2)
