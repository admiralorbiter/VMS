...
def test_user(app):
    with app.app_context():
        user = User(username="buguser", email="buguser@example.com", password_hash="hash", first_name="Bug", last_name="User", is_admin=False)
        db.session.add(user)
        db.session.commit()
        yield user
...
def test_create_bug_report(app):
    with app.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        bug = BugReport(type=BugReportType.BUG, description="Something is broken", page_url="/test", page_title="Test Page", submitted_by=user)
        db.session.add(bug)
        db.session.commit()
...
def test_bug_report_type_enum(app):
    with app.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        bug = BugReport(type=BugReportType.DATA_ERROR, description="Data error", page_url="/data", page_title="Data Page", submitted_by=user)
        db.session.add(bug)
        db.session.commit()
...
def test_resolution_workflow(app):
    with app.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        bug = BugReport(type=BugReportType.OTHER, description="Other issue", page_url="/other", page_title="Other Page", submitted_by=user)
        db.session.add(bug)
        db.session.commit()
        # Mark as resolved
        bug.resolved = True
...
def test_field_validation(app):
    with app.app_context():
        ...
        with pytest.raises(Exception):
            bug = BugReport(type=BugReportType.BUG, description=None, page_url="/fail", page_title="Fail", submitted_by=user)
            db.session.add(bug)
            db.session.commit()
        # Page URL is required
        with pytest.raises(Exception):
            bug = BugReport(type=BugReportType.BUG, description="desc", page_url=None, page_title="Fail", submitted_by=user)
            db.session.add(bug)
            db.session.commit()
        db.session.rollback()
        db.session.delete(user)
...
def test_cascade_delete_user(app):
    with app.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        bug = BugReport(type=BugReportType.BUG, description="Cascade test", page_url="/cascade", page_title="Cascade", submitted_by=user)
        db.session.add(bug)
        db.session.commit()
        user_id = user.id
        bug_id = bug.id
