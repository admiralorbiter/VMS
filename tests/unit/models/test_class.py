...
def test_class_repr(test_class):
    ...
    assert str(test_class) == "<Class Test Class 2024 (2024)>"
    assert repr(test_class) == "<Class Test Class 2024 (2024)>"
...
def test_class_timestamps(app, test_school):
    ...
    with app.app_context():
        # Create a new class
        new_class = Class(salesforce_id="a005f000003XNa8AAG", name="Timestamp Test Class", school_salesforce_id=test_school.id, class_year=2025)
...
def test_salesforce_url_property(test_class):
    ...
    with db.session.no_autoflush:
        test_class_none = Class(name="Test Class No SF", school_salesforce_id="a015f000004XNa7AAG", class_year=2024)
...
def test_index_creation(app):
    ...
    with app.app_context():
        ...
        if not indexes and str(db.engine.url).startswith("sqlite"):
            ...
            expected_indexes = {("salesforce_id",), ("name",), ("school_salesforce_id",), ("class_year", "school_salesforce_id")}
...
def test_unique_salesforce_id(app, test_class):
    ...
    with app.app_context():
        duplicate_class = Class(
            salesforce_id=test_class.salesforce_id, name="Duplicate Class", school_salesforce_id="a015f000004XNa7AAG", class_year=2024  # Same salesforce_id
        )
...
def test_class_year_validation(app, test_school):
    ...
    with app.app_context():
        ...
        for year in valid_years:
            valid_class = Class(salesforce_id=f"a005f000003XNa{year}", name=f"Class of {year}", school_salesforce_id=test_school.id, class_year=year)
...
def test_school_foreign_key_constraint(app):
    ...
    with app.app_context():
        ...
        with pytest.raises((sa.exc.IntegrityError, sa.exc.InvalidRequestError)):
            invalid_class = Class(salesforce_id="a005f000003XNa9AAG", name="Invalid School Class", school_salesforce_id="nonexistent_id", class_year=2024)
            db.session.add(invalid_class)
            db.session.commit()
