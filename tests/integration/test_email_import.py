...
class TestEmailImportFunctionality:
    ...
    def test_volunteers_with_salesforce_ids(self, app):
        ...
        with app.app_context():
            volunteers_with_sf = (
                Volunteer.query.filter(Volunteer.salesforce_individual_id.isnot(None))
                .limit(5)
                .all()
            )
    ...
    def test_history_records_count(self, app):
        ...
        with app.app_context():
            ...
            assert (
                email_history <= total_history
            ), "Email history should not exceed total history"

    def test_email_history_records_structure(self, app):
        """Test that email history records have the expected structure"""
        with app.app_context():
            email_records = (
                History.query.filter_by(activity_type="Email").limit(3).all()
            )
        ...
        with app.app_context():
            ...
            for record in email_records:
                if record.description:
                    ...
                    assert isinstance(
                        lines, list
                    ), "Description should split into lines"
                    ...
                    for line in lines[:10]:  # Check first 10 lines
                        if line.strip().startswith("Body:"):
                            ...
                            assert isinstance(
                                body_content, str
                            ), "Body content should be string"
                        elif body_started and line.strip():
                            # This is body content
                            assert isinstance(
                                line.strip(), str
                            ), "Body line should be string"
    ...
    def test_import_history_function_exists(self):
        ...
        try:
            ...
            ), "Import function should be callable"
        except ImportError:
...
class TestEmailImportIntegration:
    ...
    def test_email_import_workflow(self, app):
        ...
        with app.app_context():
            ...
            assert all(
                isinstance(step, str) for step in workflow_steps
...
"""
        ...
        assert "This is the email body content" in " ".join(
            body_content
        ), "Should find body content"
