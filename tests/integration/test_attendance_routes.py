...
def test_attendance_filtering(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/attendance?filter=recent", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_purge_attendance(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/attendance/purge", method="DELETE", headers=auth_headers
    )
...
def test_view_teacher_details(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/attendance/view/1/teachers", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_attendance_details_events_json(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/attendance/details/events.json", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_attendance_comparison(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/attendance/compare?period1=2023&period2=2024", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
