...
def test_update_attendance_detail(client, auth_headers):
    ...
    response = safe_route_test(client, "/attendance/detail/1", method="PUT", json_data=update_data, headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_attendance_validation(client, auth_headers):
    ...
    invalid_data = {"total_students": -5, "total_teachers": "invalid"}  # Invalid negative number  # Invalid string

    response = safe_route_test(client, "/attendance/validate", method="POST", json_data=invalid_data, headers=auth_headers)
