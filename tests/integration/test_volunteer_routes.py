...
def test_volunteer_detail_view(client, auth_headers, test_volunteer):
    ...
    response = safe_route_test(
        client, f"/volunteers/view/{test_volunteer.id}", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_add_volunteer(client, auth_headers, app):
    ...
    with app.app_context():
        ...
        response = safe_route_test(
            client, "/volunteers/add", method="POST", data=data, headers=auth_headers
        )
        ...
        if response.status_code in [200, 302]:
            ...
            if volunteer:
                ...
                email = Email.query.filter_by(
                    contact_id=volunteer.id, email="new.volunteer@example.com"
                ).first()
                assert email is not None

                # Verify phone was created
                phone = Phone.query.filter_by(
                    contact_id=volunteer.id, number="123-456-7890"
                ).first()
...
def test_delete_volunteer(client, auth_headers, test_volunteer, app):
    ...
    with app.app_context():
        response = safe_route_test(
            client, f"/volunteers/delete/{test_volunteer.id}", headers=auth_headers
        )
...
def test_volunteer_search(client, auth_headers, test_volunteer, app):
    ...
    with app.app_context():
        # Test name search
        response = safe_route_test(
            client, "/volunteers?search_name=Test", headers=auth_headers
        )
        assert_route_response(response, expected_statuses=[200, 404, 500])

        # Test organization search
        response = safe_route_test(
            client, "/volunteers?org_search=Test Corp", headers=auth_headers
        )
        assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_import_volunteers(client, auth_headers, test_admin, app):
    ...
    with app.app_context():
        ...
        response = safe_route_test(
            client, "/volunteers/import", data={"file": file}, headers=auth_headers
        )
...
def test_volunteer_filtering(client, auth_headers, app):
    ...
    with app.app_context():
        response = safe_route_test(
            client, "/volunteers?filter=active", headers=auth_headers
        )
        assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_volunteer_export(client, auth_headers, app):
    ...
    with app.app_context():
        response = safe_route_test(
            client, "/volunteers/export?format=csv", headers=auth_headers
        )
        assert_route_response(response, expected_statuses=[200, 404, 500])
