...
def test_add_student(client, auth_headers):
    ...
    response = safe_route_test(client, "/students/add", method="POST", headers=auth_headers)
...
def test_edit_student(client, auth_headers):
    ...
    response = safe_route_test(client, "/students/edit/1", method="POST", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])


def test_delete_student(client, auth_headers):
    """Test deleting student"""
    response = safe_route_test(client, "/students/delete/1", method="POST", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])


def test_import_students_salesforce(client, auth_headers):
    """Test importing students from Salesforce"""
    response = safe_route_test(client, "/students/import-from-salesforce", method="POST", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])


def test_students_pagination(client, auth_headers):
    """Test students pagination"""
    response = safe_route_test(client, "/students?page=2&per_page=25", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_students_filtering(client, auth_headers):
    ...
    response = safe_route_test(client, "/students?school_id=1&grade=10", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])


def test_students_sorting(client, auth_headers):
    """Test students sorting"""
    response = safe_route_test(client, "/students?sort=name&order=asc", headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_students_error_handling(client, auth_headers):
    ...
    response = safe_route_test(client, "/students/invalid-endpoint", headers=auth_headers)
...
def test_students_data_validation(client, auth_headers):
    ...
    response = safe_route_test(client, "/students/validate", method="POST", headers=auth_headers)
