...
def test_organization_search_functionality(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations?search=test", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_edit_organization(client, auth_headers):
    ...
    response = client.post(
        "/organizations/edit/1", data=update_data, headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])
...
def test_organization_volunteers(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/volunteers", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_filtering(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations?type=Corporate&status=active", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_validation(client, auth_headers):
    ...
    response = client.post(
        "/organizations/validate", json=invalid_data, headers=auth_headers
    )
...
def test_organization_contacts(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/contacts", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_partnerships(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/partnerships", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_search_advanced(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/search?query=tech&location=city", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_comparison(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/compare?ids=1,2,3", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_profile_update(client, auth_headers):
    ...
    profile_data = {"mission": "Updated mission statement", "size": "Large"}

    response = client.put(
        "/organizations/1/profile", json=profile_data, headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_compliance(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/compliance", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_certification(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/certifications", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_collaboration(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/organizations/1/collaborate", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
