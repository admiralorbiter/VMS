...
def test_virtual_usage_report_with_status_filter(client, auth_headers):
    ...
    for status in statuses:
        response = safe_route_test(client, f'/reports/virtual/usage?status={status}', headers=auth_headers)
        assert_route_response(response, expected_statuses=[200, 404, 500])

def test_virtual_usage_report_with_date_filters(client, auth_headers):
    """Test virtual usage report with date filters"""
    # Test with date range
    response = safe_route_test(client, '/reports/virtual/usage?date_from=2024-07-01&date_to=2024-12-31', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_virtual_usage_report_export(client, auth_headers):
    """Test virtual usage report export functionality"""
    response = safe_route_test(client, '/reports/virtual/usage/export', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_virtual_usage_report_cache_functionality(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/virtual/usage?refresh=1', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_virtual_usage_report_filter_combinations(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/virtual/usage?district=Hickman Mills School District&status=successfully completed&show_all_districts=1', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_virtual_usage_report_pagination(client, auth_headers):
    """Test virtual usage report pagination"""
    response = safe_route_test(client, '/reports/virtual/usage?page=1&per_page=10', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_virtual_usage_report_sorting(client, auth_headers):
    ...
    for sort_by in sort_options:
        response = safe_route_test(client, f'/reports/virtual/usage?sort_by={sort_by}', headers=auth_headers)
        assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_virtual_usage_report_error_handling(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/virtual/usage?invalid_param=test', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_report_with_filters(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/organization?year=2024-2025', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_organization_report_excel_export(client, auth_headers):
    """Test organization report excel export"""
    response = safe_route_test(client, '/reports/organization/export', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_report_excel_export_with_filters(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/organization/export?year=2024-2025', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_organization_report_detail(client, auth_headers):
    """Test organization report detail view"""
    response = safe_route_test(client, '/reports/organization/detail/1', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_organization_report_detail_with_filters(client, auth_headers):
    """Test organization report detail with filters"""
    response = safe_route_test(client, '/reports/organization/detail/1?year=2024-2025', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_organization_report_detail_excel_export(client, auth_headers):
    """Test organization report detail excel export"""
    response = safe_route_test(client, '/reports/organization/detail/1/export', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_organization_report_detail_excel_export_with_filters(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/organization/detail/1/export?year=2024-2025', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_organization_report_invalid_org_id(client, auth_headers):
    """Test organization report with invalid organization ID"""
    response = safe_route_test(client, '/reports/organization/detail/999999', headers=auth_headers)
...
def test_organization_report_excel_invalid_org_id(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/organization/detail/999999/export', headers=auth_headers)
    assert_route_response(response, expected_statuses=[404, 500])

def test_reports_with_filters(client, auth_headers):
    """Test reports with various filters"""
    response = safe_route_test(client, '/reports?year=2024-2025&district=test', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_reports_pagination(client, auth_headers):
    """Test reports pagination"""
    response = safe_route_test(client, '/reports?page=1&per_page=10', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_reports_error_handling(client, auth_headers):
    ...
    response = safe_route_test(client, '/reports/invalid-endpoint', headers=auth_headers)
    assert_route_response(response, expected_statuses=[404, 500])
