...
def test_get_calendar_events(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar/events?start=2024-01-01&end=2024-12-31", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_with_filters(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar?type=in_person&status=confirmed", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_navigation(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar?date=2024-06-01", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_accessibility(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar?accessible=true", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_timezone_handling(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar?timezone=America/New_York", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_error_handling(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar?invalid=parameter", headers=auth_headers
    )
...
def test_calendar_pagination(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar/events?page=1&limit=20", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_calendar_sorting(client, auth_headers):
    ...
    response = safe_route_test(
        client, "/calendar/events?sort=date&order=desc", headers=auth_headers
    )
    assert_route_response(response, expected_statuses=[200, 404, 500])
