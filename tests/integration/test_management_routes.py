...
def test_create_google_sheet(client, auth_headers):
    ...
    response = client.post("/google-sheets/create", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
...
def test_get_google_sheet(client, auth_headers):
    ...
    google_sheet = GoogleSheet(academic_year="2023-2024", sheet_id="test_sheet_id", created_by=user.id)  # Required parameter  # Required parameter
    db.session.add(google_sheet)
    db.session.commit()

    # Test getting Google Sheet
    response = safe_route_test(client, f"/google-sheets/{google_sheet.id}", headers=auth_headers)
...
def test_create_bug_report(client, auth_headers):
    ...
    data = {"type": BugReportType.BUG.value, "description": "New bug report", "page_url": "http://test.com"}
    response = client.post("/admin/create-bug-report", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
    assert response.status_code in [200, 400, 500, 404]
...
def test_resolve_bug_report(client, auth_headers):
    ...
    response = client.post(f"/admin/resolve-bug-report/{bug_report.id}", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
    assert response.status_code in [200, 400, 500, 404]
...
def test_create_school(client, auth_headers):
    ...
    response = client.post("/admin/create-school", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
    assert response.status_code in [200, 400, 500, 404]
...
def test_edit_school(client, auth_headers):
    ...
    response = client.post(f"/admin/edit-school/{school.id}", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
    assert response.status_code in [200, 400, 500, 404]
...
def test_management_data_validation(client, auth_headers):
    ...
    response = client.post("/admin/create-user", data=json.dumps(data), headers={**auth_headers, "Content-Type": "application/json"})
