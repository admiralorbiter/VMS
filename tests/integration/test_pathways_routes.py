...
def test_add_pathway(client, auth_headers):
    ...
    response = safe_route_test(client, '/pathways/add', method='POST', headers=auth_headers)
...
def test_edit_pathway(client, auth_headers):
    ...
    response = safe_route_test(client, '/pathways/edit/1', method='POST', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])

def test_delete_pathway(client, auth_headers):
    """Test deleting pathway"""
    response = safe_route_test(client, '/pathways/delete/1', method='POST', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])

def test_import_pathways_salesforce(client, auth_headers):
    """Test importing pathways from Salesforce"""
    response = safe_route_test(client, '/pathways/import-from-salesforce', method='POST', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])

def test_import_pathway_participants(client, auth_headers):
    """Test importing pathway participants from Salesforce"""
    response = safe_route_test(client, '/pathways/import-participants-from-salesforce', method='POST', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 302, 404, 500])

def test_pathways_pagination(client, auth_headers):
    """Test pathways pagination"""
    response = safe_route_test(client, '/pathways?page=2&per_page=20', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_pathways_filtering(client, auth_headers):
    ...
    response = safe_route_test(client, '/pathways?status=Active&type=STEM', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])

def test_pathways_sorting(client, auth_headers):
    """Test pathways sorting"""
    response = safe_route_test(client, '/pathways?sort=name&order=asc', headers=auth_headers)
    assert_route_response(response, expected_statuses=[200, 404, 500])
...
def test_pathways_error_handling(client, auth_headers):
    ...
    response = safe_route_test(client, '/pathways/invalid-endpoint', headers=auth_headers)
...
def test_pathways_data_validation(client, auth_headers):
    ...
    response = safe_route_test(client, '/pathways/validate', method='POST', headers=auth_headers)
