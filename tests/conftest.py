...
from models.contact import EducationEnum, GenderEnum, LocalStatusEnum, RaceEthnicityEnum, SkillSourceEnum
...
from models.volunteer import Engagement, EventParticipation, Skill, Volunteer, VolunteerSkill
...
def mock_template_rendering(app):
    ...
    def mock_render_template(template_name, **kwargs):
        ...
        return json.dumps({"template": template_name, "data": {k: str(v) for k, v in kwargs.items() if k != "request"}})
...
def test_user(app):
    user = User(
        username="testuser", email="test@example.com", password_hash=generate_password_hash("password123"), first_name="Test", last_name="User", is_admin=False
    )
...
def test_contact(app):
    with app.app_context():
        ...
        phone = Phone(number="123-456-7890", type=ContactTypeEnum.personal, primary=True)
        email = Email(email="john.doe@example.com", type=ContactTypeEnum.personal, primary=True)
        address = Address(
            address_line1="123 Main St", city="Kansas City", state="MO", zip_code="64111", country="USA", type=ContactTypeEnum.personal, primary=True
        )
...
def test_class(app, test_school):
    ...
    with app.app_context():
        ...
        test_class = Class(salesforce_id="a005f000003XNa7AAG", name="Test Class 2024", school_salesforce_id=school.id, class_year=2024)
...
def test_district(app):
    ...
    with app.app_context():
        district = District(salesforce_id="DIST001", name="Test District")  # Use salesforce_id for string identifier
...
def test_volunteer_skill(app, test_volunteer, test_skill):
    ...
    with app.app_context():
        vol_skill = VolunteerSkill(volunteer_id=test_volunteer.id, skill_id=test_skill.id, source=SkillSourceEnum.self_reported, interest_level="High")
...
def test_engagement(app, test_volunteer):
    ...
    with app.app_context():
        engagement = Engagement(volunteer_id=test_volunteer.id, engagement_date=date(2024, 1, 1), engagement_type="Meeting", notes="Test engagement notes")
...
def test_event_participation(app, test_volunteer, test_event):
    ...
    with app.app_context():
        participation = EventParticipation(
            volunteer_id=test_volunteer.id, event_id=test_event.id, status="Attended", delivery_hours=2.5, salesforce_id="a005f000003TEST789"
        )
...
def test_history(app, test_volunteer):
    ...
    with app.app_context():
        history = History(
            volunteer_id=test_volunteer.id, activity_date=datetime.now(), activity_type="Test Activity", notes="Test history notes", is_deleted=False
        )
...
def test_student_no_school(app):
    ...
    with app.app_context():
        student = Student(first_name="Independent", last_name="Student", current_grade=10, racial_ethnic=RaceEthnicityEnum.white)
        db.session.add(student)
        db.session.commit()
        yield student
...
def test_volunteer_with_relationships(app, test_volunteer):
    ...
    with app.app_context():
        # Add email
        email = Email(email="test.volunteer@example.com", type="personal", primary=True, contact_id=test_volunteer.id)

        # Add phone
        phone = Phone(number="123-456-7890", type="personal", primary=True, contact_id=test_volunteer.id)
