...
## TD-036: Exact-Name Duplicate Teacher Records — ✅ Effectively Resolved

**Created:** 2026-03-13 · **Priority:** Low · **Updated:** 2026-05-03

**Live database analysis (2026-05-03):** Zero active duplicate groups found. Checked via:
- `teacher_progress.name` — 0 name groups mapping to multiple distinct `teacher_id`s (active)
- Email match — only 42 of 2,470 active teachers have `cached_email`; 0 email collisions
- EventTeacher impact — 0 risky pairs (both sides with ET records)

**Summary:** The March 2026 pruning of 7,660 orphaned teachers (soft-delete tagged `pruned_20260313`) resolved the active duplicate problem. There are currently **2,470 active teachers, 19,697 EventTeacher records, and 0 duplicate name groups with EventTeacher impact**.

**What remains:**
- ~11 maiden/married name candidates flagged in March 2026 remain for manual review via the merge UI at `/teachers/merge`. These are low-risk edge cases, not data integrity threats.
- The soft-deleted pool (7,660 teachers) awaits hard-delete — see **TD-037** (re-deferred to 2026-05-13).

**Recommended action:** Close TD-036. Add to Resolved Archive after TD-037 hard-delete is executed. No merge script run needed — the pruning already resolved the problem.

---

---

## TD-037: Hard-Delete Pruned Teachers *(Re-deferred to 2026-05-13)*

**Created:** 2026-03-13 · **Priority:** Low

7,660 teachers were soft-deleted (marked `active=False`, tagged `pruned_20260313`) on 2026-03-13. Originally scheduled for hard-delete after April 13, 2026 — re-deferred to **May 13, 2026** (no issues reported, precautionary extension). Prune log: `data/prune_log_20260313_233506.json`.
...
| 7 | **~~TD-016~~** | ~~Cache Model Proliferation in `reports.py`~~ | M | ✅ Closed May 2026 |
| 8 | **TD-022** | No Test Coverage for Extracted Blueprints | M | Pending |
| 9 | **~~TD-034~~** | ~~Salesforce Data Quality Audit~~ | M | ✅ Resolved 2026-03-13 |
| 10 | **~~TD-036~~** | ~~Exact-Name Duplicate Teacher Records~~ | M | ✅ Effectively Resolved 2026-05-03 — live DB shows 0 active duplicate groups; ~11 maiden/married edge cases remain for manual merge UI review |
...
**Proposed fix:** Extract the condition builder into a shared helper function — e.g., `_build_district_event_conditions(district)` — in `routes/reports/district_year_end/routes.py` or move to `computation.py` if it's also needed there. The function should accept a `District` object and return the `db.or_(*conditions)` clause.
